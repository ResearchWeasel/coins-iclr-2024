...
class Subgraphing(MetricSet):
    def __init__(self, graph, num_nodes, community_resolution_disjoint):
    ...
    def recursive_updates(self, node_batch: pd.DataFrame, edge_batch: pd.DataFrame):
        ...
        if len(nodes_in_larger_wcc) != self.num_nodes:
            ...
            disjoint_partitioning[nodes_in_larger_wcc] = np.array(larger_wcc_comms.membership) + small_wcc_community + 1
